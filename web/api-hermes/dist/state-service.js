...
export class StateService {
    ...
    async getTrustedSkills() {
        ...
        try {
            ...
            if (!Array.isArray(parsed?.records))
                return [];
            return parsed.records.filter((r) => typeof r.path === "string" &&
    ...
    async removeTrustedSkill(skillPath) {
        if (!this.isConfigured())
            return false;
        try {
            const raw = await fs.readFile(this.getTrustedSkillsPath(), "utf8");
            ...
            parsed.records = parsed.records.filter((r) => path.resolve(r.path) !== normalizedTarget);
            if (parsed.records.length === before)
                return false;
        ...
        try {
            ...
            return {
                ...
                protectedRoots: Array.isArray(parsed.protectedRoots)
                    ? parsed.protectedRoots.filter((r) => typeof r === "string")
                    : [],
    ...
    matchesFilter(event, filters) {
        ...
        if (filters?.result) {
            if (filters.result === "blocked" && !event.blocked)
                return false;
            if (filters.result === "observed" && (event.blocked || event.mode === "off"))
                return false;
            if (filters.result === "clear" && event.mode !== "off")
                return false;
        }
        return true;
    }
    async countDefenseEvents() {
        if (!this.isConfigured())
            return 0;
        try {
            const raw = await fs.readFile(this.getDefenseEventsPath(), "utf8");
