...
import { ConfigService, AegisConfig } from "../config-service.js";
  ...
  router.get("/", async (_req, res, next) => {
    try {
      ...
      if (rpcClient?.isReady()) {
        ...
        res.json({
          ok: true,
          data: { config: runtimeConfig as AegisConfig, defaults: CONFIG_DEFAULTS },
        });
        return;
      }
    ...
    try {
      ...
      if (rpcClient?.isReady()) {
        try {
          ...
          console.log(
            "[aegis-web] Config updated. Note: A Hermes plugin restart may be needed for all changes to take effect."
          );
        } catch {
