...
const CONFIG_DEFAULTS: AegisConfig = {
...
export class ConfigService {
  ...
  private async writeConfigFile(data: Record<string, unknown>): Promise<void> {
  ...
  resolveConfig(userConfig: Record<string, unknown>): AegisConfig {
    const allDefensesEnabled =
      userConfig.allDefensesEnabled !== false
    ...
    const selfProtectionMode = readMode(
      "selfProtectionEnabled",
      "selfProtectionMode"
    );
    const commandBlockMode = readMode(
      "commandBlockEnabled",
      "commandBlockMode"
    );
    const encodingGuardMode = readMode(
      "encodingGuardEnabled",
      "encodingGuardMode"
    );
    const scriptProvenanceGuardMode = readMode(
    ...
    const exfiltrationGuardMode = readMode(
      "exfiltrationGuardEnabled",
      "exfiltrationGuardMode"
    );
    const dispatchGuardMode = readMode(
